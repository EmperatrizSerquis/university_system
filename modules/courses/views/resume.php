<div class="w3-row">
    <div class="w3-container">
        ...
        <?= flashdata() ?>
        <p>
            <a href="<?= BASE_URL ?>courses/create"><button class="w3-button w3-medium primary">
                <i class="fa fa-pencil"></i> CREATE NEW RECORD</button>
            </a>
            <a href="<?= BASE_URL ?>courses/manage"><button class="w3-button w3-medium primary">
                <i class="fa fa-pencil"></i> VIEW MANAGE COURSES</button>
            </a>
        </p>
...
function refreshResults(searchPhrase) {
  //update totalRows
    var params = {
      'course_title LIKE': '%' + searchPhrase + '%'
    }

  var target_url = '<?= BASE_URL ?>api/count/courses';
...
function noResults() {
  ...
  if (searchPhrase !== '') {
    ...
    var msg = 'There are currently no course records';
  }
...
function getRecords() {
  ...
  var target_url = '<?= BASE_URL ?>courses/api_lecturer';
  ...
  http.onload = function () {
    ...
    for (var i = 0; i < records.length; i++) {
      records[i];
      var recordUrl = '<?= BASE_URL ?>courses/show/' + records[i]['id'];
      var editBtn = '<a href="' + recordUrl + '"><button type="button" class="btn btn-xs">View</button></a>';
      var newRow = `<tr>
                        <td>${records[i]['course_title']}</td>
                        <td>${records[i]['start_date']}</td>
                        <td>${records[i]['finish_date']}</td>
                        <td>${records[i]['started_time']}</td>
                        <td>${records[i]['last_name']}, ${records[i]['first_name']}</td>
                        <td>${editBtn}</td>
                    </tr>`;
