<?php
class Associated_courses_and_lecturers extends Trongate {

    private $associated_modules = ['courses', 'lecturers'];
    private $assoc_module = 'associated_courses_and_lecturers';
    private $relationshipType = 'many to many';
    private $bridgingTblRequired = 'true';

    function _get_identifier_column($module_name) {

        if ($module_name == 'courses') {
            $identifier_column = 'course_title';
        } else {
            $identifier_column = 'first_name,last_name';
        }
    ...
    function _draw_association_info($token) {
        ...
        if ($data['calling_module'] == 'courses') {
            $data['associated_records_singular'] = 'Lecturer ';
            $data['associated_records_plural'] = 'lecturers';
        } else {
            $data['associated_records_singular'] = 'Course ';
            $data['associated_records_plural'] = 'courses';            
        }
    ...
    function fetch($update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                SELECT
                    associated_courses_and_lecturers.id as recordId,
                    associated_module_name.id ,
                    CONCAT(xxxx) as identifierColumn 
                FROM
                    associated_module_name
                LEFT JOIN associated_courses_and_lecturers ON associated_module_name.id = associated_courses_and_lecturers.associated_module_name_id
                WHERE
                    associated_courses_and_lecturers.calling_module_name_id = '.$update_id;
    ...
    function _build_sql_statement($identifier_column, $associated_module_name, $update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                ...
                FROM
                '.$associated_module_name.'
                LEFT JOIN associated_courses_and_lecturers                ON '.$associated_module_name.'.id = associated_courses_and_lecturers.'.$associated_module_name.'_id 
                WHERE associated_courses_and_lecturers.'.$associated_module_name.'_id IS NULL';
        ...
        if ($relationshipType == 'many to many') {
            ...
            $ditch = 'WHERE associated_courses_and_lecturers.'.$associated_module_name.'_id IS NULL';
            $replace = 'WHERE associated_courses_and_lecturers.'.$calling_module_name.'_id >0 ';
            $replace.= 'OR associated_courses_and_lecturers.'.$calling_module_name.'_id IS NULL';
    ...
    function _get_associated_module_name($calling_module_name) {
        ...
        if ($calling_module_name == 'courses') {
            $associated_module_name = 'lecturers';
        } else {
            $associated_module_name = 'courses';
        }
    ...
    function _prep_inbound($input) {
        ...
        if ($params['callingModule'] == 'lecturers') {
            $data['courses_id'] = $params['selectedValue'];
            $data['lecturers_id'] = $params['update_id'];
        } else {
            $data['lecturers_id'] = $params['selectedValue'];
            $data['courses_id'] = $params['update_id'];
        }
