<?php
class Associated_students_and_lecturers extends Trongate {

    private $associated_modules = ['students', 'lecturers'];
    private $assoc_module = 'associated_students_and_lecturers';
    ...
    function _draw_association_info($token) {
        ...
        if ($data['calling_module'] == 'students') {
            $data['associated_records_singular'] = 'Lecturer ';
            $data['associated_records_plural'] = 'lecturers';
        } else {
            $data['associated_records_singular'] = 'Student ';
            $data['associated_records_plural'] = 'students';            
        }
    ...
    function fetch($update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                SELECT
                    associated_students_and_lecturers.id as recordId,
                    associated_module_name.id ,
                    CONCAT(xxxx) as identifierColumn 
                FROM
                    associated_module_name
                LEFT JOIN associated_students_and_lecturers ON associated_module_name.id = associated_students_and_lecturers.associated_module_name_id
                WHERE
                    associated_students_and_lecturers.calling_module_name_id = '.$update_id;
    ...
    function _build_sql_statement($identifier_column, $associated_module_name, $update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                ...
                FROM
                '.$associated_module_name.'
                LEFT JOIN associated_students_and_lecturers                ON '.$associated_module_name.'.id = associated_students_and_lecturers.'.$associated_module_name.'_id 
                WHERE associated_students_and_lecturers.'.$associated_module_name.'_id IS NULL';
        ...
        if ($relationshipType == 'many to many') {
            ...
            $ditch = 'WHERE associated_students_and_lecturers.'.$associated_module_name.'_id IS NULL';
            $replace = 'WHERE associated_students_and_lecturers.'.$calling_module_name.'_id >0 ';
            $replace.= 'OR associated_students_and_lecturers.'.$calling_module_name.'_id IS NULL';
    ...
    function _get_associated_module_name($calling_module_name) {
        ...
        if ($calling_module_name == 'students') {
            $associated_module_name = 'lecturers';
        } else {
            $associated_module_name = 'students';
        }
