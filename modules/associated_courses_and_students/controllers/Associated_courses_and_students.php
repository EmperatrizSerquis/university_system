<?php
class Associated_courses_and_students extends Trongate {

    private $associated_modules = ['courses', 'students'];
    private $assoc_module = 'associated_courses_and_students';
    ...
    function fetch($update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                SELECT
                    associated_courses_and_students.id as recordId,
                    associated_module_name.id ,
                    CONCAT(xxxx) as identifierColumn 
                FROM
                    associated_module_name
                LEFT JOIN associated_courses_and_students ON associated_module_name.id = associated_courses_and_students.associated_module_name_id
                WHERE
                    associated_courses_and_students.calling_module_name_id = '.$update_id;
    ...
    function _build_sql_statement($identifier_column, $associated_module_name, $update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                ...
                FROM
                '.$associated_module_name.'
                LEFT JOIN associated_courses_and_students                ON '.$associated_module_name.'.id = associated_courses_and_students.'.$associated_module_name.'_id 
                WHERE associated_courses_and_students.'.$associated_module_name.'_id IS NULL';
        ...
        if ($relationshipType == 'many to many') {
            ...
            $ditch = 'WHERE associated_courses_and_students.'.$associated_module_name.'_id IS NULL';
            $replace = 'WHERE associated_courses_and_students.'.$calling_module_name.'_id >0 ';
            $replace.= 'OR associated_courses_and_students.'.$calling_module_name.'_id IS NULL';
    ...
    function _prep_inbound($input) {
        ...
        if ($params['callingModule'] == 'students') {
